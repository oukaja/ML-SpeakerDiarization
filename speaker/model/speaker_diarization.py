...
import scipy.signal
from speaker.utils.feature_extraction import *
from speaker.utils.utils import *
from speaker.utils.vad import *
from speaker.model.unsupervised_model import kmeans_silhouette
...
def speaker_diarization(fs, signal, mt_size=2.0, mt_step=0.2, st_win=0.05):
    ...
    for i in range(1, len(cls)):
        if cls[i] == cls[i-1]:
            ...
            if not os.path.exists(os.path.join( "downloads/" + datetime.date.today().strftime("%B-%d-%Y-%H-%M-%S%p"))):
                os.makedirs(os.path.join( "downloads/" + datetime.date.today().strftime("%B-%d-%Y-%H-%M-%S%p")))
